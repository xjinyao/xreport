package com.xjinyao.report.action.html;

import com.xjinyao.report.action.MobileUtils;
...
public class HtmlPreviewServletAction extends RenderPageServletAction {
	...
	public void execute(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (method != null) {
			...
			Template template = ve.getTemplate("xreport-html/html-preview.html", "utf-8");
	...
	private String buildTitle(HttpServletRequest req) {
		...
		if (StringUtils.isBlank(title)) {
			...
			title = decode(title);
			int point = title.lastIndexOf(".xreport.xml");
			if (point > -1) {
