package com.xjinyao.report.core.export.word.high;
...
import com.xjinyao.report.core.export.Producer;
import com.xjinyao.report.core.export.word.DxaUtils;
...
public class WordProducer implements Producer {
	public static final String BEAN_ID = "xreport.wordExporter";
			...
			if (StringUtils.isNotBlank(base64Data)) {
				...
				try {
					...
					if (imageType.equals("jpeg")) {
						run.addPicture(inputStream, XWPFDocument.PICTURE_TYPE_JPEG, "xreport-" + rowNumber + "-" + columnNumber + ".jpg", Units.toEMU(width), Units.toEMU(height));
					} else if (imageType.equals("png")) {
						run.addPicture(inputStream, XWPFDocument.PICTURE_TYPE_PNG, "xreport-" + rowNumber + "-" + columnNumber + ".png", Units.toEMU(width), Units.toEMU(height));
					} else if (imageType.equals("gif")) {
						run.addPicture(inputStream, XWPFDocument.PICTURE_TYPE_GIF, "xreport-" + rowNumber + "-" + columnNumber + ".gif", Units.toEMU(width), Units.toEMU(height));
					}
				} catch (Exception ex) {
					throw new ReportComputeException(ex);
				} finally {
