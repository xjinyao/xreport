...
export function showLoading() {
    const url = window._server + '/res/xreport-asserts/icons/loading.gif';
    const h = $(window).height() / 2, w = $(window).width() / 2;
    const cover = $(`<div class="xreport-loading-cover" style="position: absolute;left: 0px;top: 0px;width:${w * 2}px;height:${h * 2}px;z-index: 1199;background:rgba(222,222,222,.5)"></div>`);
    $(document.body).append(cover);
    const loading = $(`<div class="xreport-loading" style="text-align: center;position: absolute;z-index: 1120;left: ${w - 35}px;top: ${h - 35}px;"><img src="${url}">
    <div style="margin-top: 5px">打印数据加载中...</div></div>`);
    $(document.body).append(loading);
};

export function hideLoading() {
    $('.xreport-loading-cover').remove();
    $('.xreport-loading').remove();
};
...
export function tableToXml(context) {
    ...
    let xml = `<?xml version="1.0" encoding="UTF-8"?><xreport>`;
...
export function windowOpen(url, target, params) {
    if (params) {
        Object.keys(params).forEach(k => {
            url += (url.indexOf("?") !== -1 ? "&" : "?") + k + "=" + params[k];
        });
    }
