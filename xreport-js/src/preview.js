...
import {getParameter, hideLoading, pointToMM, showLoading, windowOpen} from './Utils.js';
...
$(document).ready(function () {
    let authorization = getParameter("au");
    let projectId = getParameter("projectId");
    $.ajaxSetup({
        headers: {
            "Authorization": "Bearer " + decodeURIComponent(authorization),
            "projectId": decodeURIComponent(projectId),
        },
    });
    window.defaultUrlParams = {
        au: authorization,
        projectId: projectId,
    }
        ...
        $.ajax({
            ...
            success: function (result) {
                $.get(window._server + '/preview/loadPagePaper' + urlParameters, function (paper) {
                    ...
                    styles += $('#_xreport_table_style').html();
                    styles += `</style>`;
    ...
    $(`.xreport-pdf-print`).click(function () {
        const urlParameters = buildLocationSearchParameters();
        $.get(window._server + '/preview/loadPagePaper' + urlParameters, function (paper) {
            pdfPrintDialog.show(paper);
        });
    });
    $(`.xreport-pdf-direct-print`).click(function () {
        showLoading();
        const urlParameters = buildLocationSearchParameters();
    ...
    $(`.xreport-export-pdf`).click(function () {
        const urlParameters = buildLocationSearchParameters();
        const url = window._server + '/pdf' + urlParameters;
        // window.open(url,'_blank');
        windowOpen(url, "_blank", window.defaultUrlParams)
    });
    $(`.xreport-export-word`).click(function () {
        const urlParameters = buildLocationSearchParameters();
        const url = window._server + '/word' + urlParameters;
        // window.open(url,'_blank');
        windowOpen(url, "_blank", window.defaultUrlParams)
    });
    $(`.xreport-export-excel`).click(function () {
        const urlParameters = buildLocationSearchParameters();
        const url = window._server + '/excel' + urlParameters;
        // window.open(url,'_blank');
        windowOpen(url, "_blank", window.defaultUrlParams)
    });
    $(`.xreport-export-excel-paging`).click(function () {
        const urlParameters = buildLocationSearchParameters();
        const url = window._server + '/excel/paging' + urlParameters;
        // window.open(url,'_blank');
        windowOpen(url, "_blank", window.defaultUrlParams)
    });
    $(`.xreport-export-excel-paging-sheet`).click(function () {
        const urlParameters = buildLocationSearchParameters();
...
window.buildPaging = function (pageIndex, totalPage) {
    ...
    pageSelector.change(function () {
        ...
        windowOpen(url, "_self", window.defaultUrlParams)
    });
...
function _refreshData(second) {
    ...
    $.ajax({
        ...
        success: function (report) {
            const tableContainer = $(`#_xreport_table`);
...
window.submitSearchForm = function (file, customParameters, searchParams, callback) {
