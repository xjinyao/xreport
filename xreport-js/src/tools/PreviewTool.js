...
import {tableToXml, windowOpen} from '../Utils.js';
...
export default class PreviewTool extends Tool {
    ...
    buildButton() {
        ...
        const mainBtn = $(`<button type="button" class="btn btn-default dropdown-toggle" style="border:none;border-radius:0;background: #f8f8f8;padding: 6px 5px;" data-toggle="dropdown" title="${window.i18n.tools.preview.preview}">
            <i class="xreport xreport-preview" style="color: #0e90d2;"></i>
            <span class="caret"></span>
        </button>`);
        const ul = $(`<ul class="dropdown-menu" role="menu"></ul>`);
        const preview = $(`<li>
                <a href="###">
                    <i class="xreport xreport-preview" style="color: #0e90d2;"></i> ${window.i18n.tools.preview.view}
                </a>
            </li>`);
        ul.append(preview);
        const _this = this;
    ...
    doPreview(withPaging) {
        ...
        $.ajax({
            url: window._server + "/designer/savePreviewData?cid=" + id,
            type: 'POST',
            data: {content},
            success: function () {
                let newWindow = windowOpen(targetUrl, "_blank", urlParams);
                newWindow.focus();
            },
            error: function () {
    ...
    getIcon() {
        return `<i class="xreport xreport-preview" style="color: #0e90d2;"></i>`;
    }
}