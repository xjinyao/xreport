...
export default class BuildinTree extends BaseTree {
    ...
    _buildDatasource() {
        ...
        const rootSpan = $(`
            <span id="${this.id}">
                <i class='xreport xreport-minus' style='margin-right:2px'></i>
                <i class="xreport xreport-shareconnection"></i> <a href='###' class="ds_name">${this.name}</a>
            </span>`);
    ...
    addDataset(dataset) {
        ...
        const span = $(`<span id="${spanId}"><i class='xreport xreport-minus' style='margin-right:2px'></i> <i class="xreport xreport-sqlds"></i> <a href='###' class="dataset_name">${dataset.name}</a></span>`);
        li.append(span);
    ...
    addField(dataset, fields, field, ul) {
        ...
        const li = $(`<li></li>`);
        const spanId = uuid.v1();
        const span = $(`<span id="${spanId}" title="${window.i18n.tree.doubleClick}"><i class="xreport xreport-property"></i> <a href='###'>${field.name}</a></span>`);
        li.append(span);
    ...
    attachEvent(span, li) {
        span.click(function (e) {
            ...
            if ($liChildren.is(":visible")) {
                ...
                span.children('i:first').addClass('xreport-plus').removeClass('xreport-minus');
            } else {
                $liChildren.show('fast');
                span.children('i:first').addClass('xreport-minus').removeClass('xreport-plus');
            }
            e.stopPropagation();
        });
    }
